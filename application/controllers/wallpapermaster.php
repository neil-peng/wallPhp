...
class Wallpapermaster extends CI_Controller {

	function __construct()
	{
		parent::__construct();
		$this->load->model('displayModel/imgurl_model','iM');
		$this->load->model('accountModel/typepro_model','tP');
	...
	{
		$pid =  $_POST['pictureId'];
		$type = $_POST['pictureType'];
		$ret = "success type";
		$typeId = $this->tP->translateTypeId($type);
		if($typeId<0)
			$type = "err type" ;
		$this->tP->addType($pid,$typeId);
		echo($type);
	}
	...
	public function showPage($pageIndex)
	{
		...
		if($pageIndex>0)
		{
			...
			for ($i=0; $i <12 ; $i++) { 
				...
				{
					$data['srcUrl'][$i][3] = $this->tP->translateType($getType);;
				}
			}
			...
			//var_dump($data);
			$this->load->view('wall/headmaster',$data);
